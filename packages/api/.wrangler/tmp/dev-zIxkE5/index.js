...
// .wrangler/tmp/bundle-c1HtSf/checked-fetch.js
...
// .wrangler/tmp/bundle-c1HtSf/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-c1HtSf/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-c1HtSf/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
app.put("/api/projects/:id", async (c) => {
  try {
    const id = c.req.param("id");
    const { name: name2, description, status } = await c.req.json();
    const adapter = new PrismaD1(c.env.DB);
    const prisma = new import_client.PrismaClient({ adapter });
